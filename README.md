...
### Extract a 10kb window up- and down-stream of your specified gene/s
```bash
python slice_multi_gbk.py -i infile.gbk -g gene1: -w 10000 -o gene_sliced_output # using gene names
python slice_multi_gbk.py -i infile.gbk -g gene1:gene2 -w 10000 -o gene_sliced_output # using gene names
python slice_multi_gbk.py -i infile.gbk -lt locus_tag1:locus_tag2 -w 10000 -o locus_tag_sliced_output # using locus tags
```
...
| `-w`/`--window`  | Number of basepairs to slice before and after the selected gene/s. Will take maximum number of basepairs up/downstream if at end of a contig. Used with `-g` or `-lt`. Default=0  | 500 bp up- and downstream: `-w 500`  |
